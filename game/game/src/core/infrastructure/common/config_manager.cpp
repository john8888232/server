...
bool ConfigManager::reloadGameConfigFromRedis(const std::string& gameType) {
    try {
        ...
        // 通知游戏配置已更新
        gameRegistry->onGameConfigUpdated(gameType);
        LOG_INFO("Notified game registry of config update for %s", gameType.c_str());
        
        return true;
    } catch (const std::exception& e) {
        LOG_ERROR("Error reloading game config from Redis: %s", e.what());
        return false;
    }
}
