...
#include "core/application/services/user_balance_service.h"
...
private:
    ...
    // 用户余额服务
    std::shared_ptr<UserBalanceService> userBalanceService_;
    ...
    // 收集余额更新数据
    std::vector<PlayerBalanceUpdate> collectBalanceUpdates();
    
    // 处理余额更新结果
    void processBalanceUpdateResults(const std::vector<PlayerBalanceUpdateResult>& results);
};
