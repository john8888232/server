...
private:
    bool initialize();
    void sendErrorResponse(const std::string& sessionId, int errorCode, const std::string& roundId, double balance, int playType = 0);
    
    ResponseCallback responseCallback_;
    std::shared_ptr<MinesGameService> gameService_;
    std::shared_ptr<IUserBalanceRepository> userBalanceRepository_;
